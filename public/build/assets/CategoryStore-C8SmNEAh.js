import { d as defineStore } from "./app-LWwlYPSu.js";
...
const useCategoryStore = defineStore("CategoryStore", {
  ...
  actions: {
    ...
    initSeoTags() {
      ...
      this.category.meta_tag_description = this.category.description.replace(/<\/?[^>]+(>|$)/g, "");
    }
  },
  getters: {
    ...
    showSeoAlert: (state) => {
      return () => {
        if (state.category.meta_tag_title && state.category.meta_tag_title.length) {
          return false;
        }
        if (state.category.name.length > 1 && state.category.description.length > 2) {
          return false;
        }
        return true;
      };
